...
global_word_list = {} #dictionary for storing words and the page names they have occured
global_list_of_pages = {} #dictionary for storing pages per thier page names
...
def searchMain():
    ...
    """
    forms the main search function performing the following tasks:
    1. Loops through the global list of queries :
        assigns a dictionary and visited array (to avoid repetitive checking)
        2. Loops through each word in query and checks if the word is present or not in global word list
            if not present in global list, word not present in any page. Continues to next word in query.
            if present:
            3. loops through each page's word list that the word contains , accessed from global word dict( value)
            and sends page and query to check SOP.
            4. assigns returned result to the dictionary, which is sorted in the following order :
                sorted in descending order of value(SOP). If values matched then sorted in ascending order of keys(page names).
            5. sends to print function
...
def readFromFile():
        ...
        Name the file as input and in txt format. See Seach/input.txt for example
        """
